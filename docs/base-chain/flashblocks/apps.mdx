...
To use Flashblocks with Wagmi, you will need to add the RPC endpoint to the `transports` property of the Wagmi Config (see `config.ts`).
...
// Create client with the Flashblocks-aware RPC endpoint.
